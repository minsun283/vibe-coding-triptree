...
function ProductEditPage() {
  ...
  useEffect(() => {
    ...
    fetchProduct()
  }, [isAuthChecked, isAdmin, id])

  const handleImageUpload = useCallback((imageUrl) => {
    setForm((prev) => (prev ? { ...prev, image: imageUrl } : prev))
  }, [])

  const handleThumbnailUpload = useCallback((imageUrl) => {
    setForm((prev) => (prev ? { ...prev, thumbnail: imageUrl } : prev))
  }, [])
  ...
  const handleGroupTypeToggle = (groupType) => {
    setForm((prev) => {
      if (!prev) {
        return prev
      }

      const isSelected = prev.groupType.includes(groupType)
      const nextGroupType = isSelected
        ? prev.groupType.filter((item) => item !== groupType)
        : [...prev.groupType, groupType]

      return { ...prev, groupType: nextGroupType }
    })
  }
  ...
  const validateForm = () => {
    if (!form.sku.trim()) return 'SKU 정보가 없습니다.'
    ...
    if (!form.location.trim()) return '장소를 입력해 주세요.'
    if (!form.recommendedSeason.length) return '추천 계절을 하나 이상 선택해 주세요.'
    if (!form.groupType.length) return '단체 유형을 하나 이상 선택해 주세요.'
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const data = await updateProduct(id, buildProductPayload())
    ...
    return (
      <div className="product-create-page">
        ...
        <main className="product-create-page__content">
          <p className="product-create-error">상품 정보를 불러오는 중...</p>
        </main>
      </div>
    )
  }
  ...
  if (!form) {
    return (
      <div className="product-create-page">
        ...
        <main className="product-create-page__content">
          <Link to="/admin/products" className="product-create-back">
            <ArrowLeft size={18} />
            상품 관리로 돌아가기
          </Link>
          <p className="product-create-error">{error || '상품을 찾을 수 없습니다.'}</p>
        </main>
      </div>
    )
  }
      ...
      <main className="product-create-page__content">
        ...
        <div className="product-create-card">
          <header className="product-create-header">
            <h1 className="product-create-header__title">상품 수정</h1>
          ...
          <form className="product-create-form" onSubmit={handleSubmit}>
            <div className="product-create-form__grid">
              <div className="product-create-field">
                ...
                />
                <p className="product-create-field__hint">수정할 수 없는 고유 SKU입니다.</p>
              </div>
              ...
              <div className="product-create-field product-create-field--full">
                <span className="product-create-field__label">단체 유형</span>
                <div className="product-create-option-group" role="group" aria-label="단체 유형">
                  {GROUP_TYPES.map((type) => {
                    const isSelected = form.groupType.includes(type)
                    ...
                    return (
                      <button
                        ...
                        className={`product-create-option-btn${isSelected ? ' is-selected' : ''}`}
                        aria-pressed={isSelected}
                        onClick={() => handleGroupTypeToggle(type)}
                      >
                        {type}
                      </button>
                    )
            ...
            <div className="product-create-actions">
              <Link to="/admin/products" className="product-create-btn product-create-btn--secondary">
                취소
              </Link>
              <button
                type="submit"
      ...
      {showSuccessPopup && (
        <div className="product-create-popup-overlay" onClick={handleCloseSuccessPopup}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="product-edit-popup-title"
            onClick={(event) => event.stopPropagation()}
          >
            <h2 id="product-edit-popup-title" className="product-create-popup__title">
              수정 완료
            </h2>
            <p className="product-create-popup__message">
              {updatedProduct?.name
                ? `${updatedProduct.name} 상품이 성공적으로 수정되었습니다.`
                : '상품이 성공적으로 수정되었습니다.'}
            </p>
            <button
              type="button"
