...
import BoardResources from '@/components/board/BoardResources'
...
import { BOARD_TABS, getBoardTabFromParam } from '@/constants/boardData'
...
function BoardPage() {
  ...
  const activeTab = getBoardTabFromParam(searchParams.get('tab'))
  ...
  const fetchPosts = useCallback(async () => {
    if (activeTab === 'reviews' || activeTab === 'resources') {
      setIsLoading(false)
      return
    }
  ...
  return (
    <BoardShell activeTab={activeTab}>
      <header
        className={`board-page__header${
          activeTab === 'reviews' || activeTab === 'resources'
            ? ' board-page__header--centered'
            : ''
        }`}
      >
      ...
      {activeTab === 'reviews' ? (
        ...
        <>
          ...
          {isLoading && <p className="board-page__status">게시글을 불러오는 중...</p>}
